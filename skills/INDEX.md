...
Layer 2: skills/                          "How OpenNolan uses these tools"
...
Layer 3: .agents/app/skills/                  "How the technology itself works"
...
3. Layer 2 skills (this directory) teach the agent OpenNolan-specific conventions
4. Layer 3 skills (`.agents/app/skills/`) provide generic API knowledge, loaded on-demand
...
Repo-local social-media mirrors live under `social-media/<skill>/SKILL.md`. They are real repo-local copies of the social-media skill set so OpenNolan agents synced through GitHub can read Instagram/Reels/carousel/marketing operating knowledge without relying on absolute paths or symlinks. See `social-media/INDEX.md` and `social-media/mirror-manifest.json`.
...
| Visual Hierarchy Edit Fix | `creative/visual-hierarchy-edit-fix.md` | Creator-led before/after edit teardowns: comment challenge -> bad edit isolate -> principle receipt -> hierarchy rebuild -> polished poster reveal | `remotion-best-practices`, `hyperframes`, `gsap-core`, `gsap-timeline` |
...
All agent skills live in `.agents/app/skills/` and are managed via `npx skills add`.
...
| **Image Generation** | `flux-best-practices`, `bfl-api`, `grok-media` | `black-forest-labs/skills`, local OpenNolan skill |
...
| **AI Video (Premium)** | `seedance-2-0` — preferred premium default (cinematic, trailer, multi-shot, lip-sync, synced audio); accessed via `seedance_video` (fal.ai) or `heygen_video` Avatar Shots | Local OpenNolan skill |
| **Infrastructure** | `acestep`, `ltx2`, `playwright-recording` | `digitalsamba/claude-code-video-toolkit` |
| **Social Media & Content Marketing** | `instagram-reels`, `instagram-carousel`, `daily-carousel-remix`, `daily-tech-carousel`, `source-backed-reel-evidence-montage`, `editorial-ai-product-design-system`, `xurl` | Local OpenNolan skill (social-media) |
