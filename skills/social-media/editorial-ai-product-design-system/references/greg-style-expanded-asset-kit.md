...
Use when the user asks to generate reusable assets/icons/backgrounds for informational AI/startup Reels.
...
Session baseline: `~/greg-style-kit-expanded` produced 28 Codex-native `$imagegen` PNGs, 30 deterministic SVG icons, 7 SVG shapes, 2 TSX templates, 129 files total, zipped to `~/greg-style-kit-expanded.zip`.
...
```bash
~/.codex/generated_images/<thread_id>/*.png
```
...
import zipfile
root = Path('~/greg-style-kit-expanded')
zp = Path('~/greg-style-kit-expanded.zip')
...
for p in (root/'ai-generated').rglob('*.png'):
    Image.open(p).verify()
with zipfile.ZipFile('~/greg-style-kit-expanded.zip') as z:
...
- `MEDIA:~/greg-style-kit-expanded.zip`
- `MEDIA:~/greg-style-kit-expanded/previews/ai-generated-contact-sheet.jpg`
