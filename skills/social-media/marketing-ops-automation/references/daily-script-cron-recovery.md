...
Use this when Het asks for "today's script" and the scheduled Instagram AI/Tech daily concept/script job failed or no `scripts/YYYY-MM-DD/daily-ai-tech-video.md` exists.

## Fast recovery pattern

1. Check the target date in America/Los_Angeles.
2. Verify whether the expected script file exists under:
   - `~/.hermes/marketing-os/scripts/YYYY-MM-DD/daily-ai-tech-video.md`
3. If missing, inspect the live cron status/output enough to know whether the scheduled job failed or is still pending.
4. Do **not** stop at "the cron failed" or only trigger the cron and wait. Het asked for the deliverable now.
5. Build the script manually from the current research corpus:
   - `~/.hermes/marketing-os/research/latest.md`
...
9. Tell Het concisely that the cron failed and you recovered the script manually; include the path and the usable talking script.
...
- Do not over-deliver internal debugging detail to Het; lead with the script and only briefly mention recovery status.
