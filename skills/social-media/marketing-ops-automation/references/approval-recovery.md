# Content OS approval recovery and late approvals

Session-derived pitfalls for approval-gated cron workflows.

## Problem observed

A user approved idea numbers after the scheduled 7 PM script job had already run. The job output said it could not find clear approvals, so no scripts existed under `~/content-os/scripts/YYYY-MM-DD/`. A later user asked for the scripts and expected them to exist.
...
1. Check live time and relevant run outputs:
   ...
   - `~/content-os/scripts/YYYY-MM-DD/`
2. If scripts are missing, do not claim the cron job created them.
3. Record the approval in `~/content-os/approvals/YYYY-MM-DD.md` with approved numbers and any approved tools/spend caps.
...
- `~/content-os/approvals/YYYY-MM-DD.md`
- `~/content-os/approvals/YYYY-MM-DD-scripts-approved.md`
