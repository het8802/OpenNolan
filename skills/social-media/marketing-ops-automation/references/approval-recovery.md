...
A user approved idea numbers after the scheduled 7 PM script job had already run. The job output said it could not find clear approvals, so no scripts existed under `~/.hermes/marketing-os/scripts/YYYY-MM-DD/`. A later user asked for the scripts and expected them to exist.

## Recovery pattern

1. Check live time and relevant run outputs:
   - `date`
   - `~/.hermes/cron/output/<script-job-id>/...md`
   - `~/.hermes/marketing-os/scripts/YYYY-MM-DD/`
2. If scripts are missing, do not claim the cron job created them.
3. Record the approval in `~/.hermes/marketing-os/approvals/YYYY-MM-DD.md` with approved numbers and any approved tools/spend caps.
...
- `~/.hermes/marketing-os/approvals/YYYY-MM-DD.md`
- `~/.hermes/marketing-os/approvals/YYYY-MM-DD-scripts-approved.md`
- recent session history via `session_search`

This avoids missed approvals when session search cannot see the exact Slack thread context or when approval came after the scheduled run.
