...
3. **Daily asset + B-roll pack**
   ...
   - Use Codex from a git repo such as `~/instagram-ai-tech-assets` to generate SVG/HTML/Python asset code, then copy finished outputs into the asset package.
...
- Leaving delivery as a stale raw channel ID can cause `channel_not_found`; prefer named targets from `send_message action=list`.
