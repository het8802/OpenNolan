...
This is the **OpenNolan-specific** guide to HyperFrames. It explains when
OpenNolan pipelines should choose HyperFrames over Remotion, how OpenNolan
...
- `.agents/app/skills/hyperframes/` — composition authoring contract + GSAP rules
- `.agents/app/skills/hyperframes-cli/` — init, lint, validate, preview, render
- `.agents/app/skills/hyperframes-registry/` — `hyperframes add` + block wiring
- `.agents/app/skills/website-to-hyperframes/` — capture-to-video workflow

This file teaches the bridge between the two.

---

## When OpenNolan should pick HyperFrames (vs Remotion vs FFmpeg)

OpenNolan separates two concepts:
...
OpenNolan artifacts into HyperFrames project files:

| OpenNolan artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. OpenNolan does **not**
...
OpenNolan playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the OpenNolan orchestrator. `init`
