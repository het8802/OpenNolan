...
- **Lane A — de-repo the runtime: DONE + committed** (`cd19a68`). `lib/app_paths.py` single source of truth; env_config/env_loader/app/agent_runner rewired; `main.js` sets `OPENNOLAN_HOME` when packaged. Dev behavior-preserving. Tests green.
- **Lane B — analytics foundation: DONE + committed** (`6014275`). `server/analytics.py` (opt-out honored at init, anonymous device id, PII scrub, no-op if disabled/absent/under-pytest) + `server/settings.py` (atomic prefs) + `/api/settings/analytics` + `app_opened`. Tests green. **TODO (next):** wire funnel events — `export_completed` (activation), `agent_turn_*`, `deps_install_*`. These touch `server/render_jobs.py`, which is in uncommitted debug-recorder WIP; wire after that lands to avoid entanglement.
- **Lane C — feedback backend: DONE + committed** (`b85b932`). `server/feedback.py` (durable local jsonl → PostHog metadata event → best-effort Resend email) + `POST /api/feedback` + RESEND_API_KEY/FEEDBACK_TO in the BYOK menu. Tests green. **TODO:** frontend feedback panel (entangles with uncommitted `web/src/studio` WIP; needs preview verification).
- **Lane D — packaging/sign/notarize/auto-update: DONE + committed** (`d1d7171`). electron-builder arm64 dmg+zip; ships backend tree + web/dist + a pruned, signed python-build-standalone interpreter; `mac.notarize:true` (electron-builder signs the nested Python + notarizes + staples); hardened-runtime entitlements (inherit plist locked); `main.js` runs the bundled backend/interpreter when packaged; dev-guarded electron-updater; `scripts/fetch-python.mjs` (sha-pinned + pruned); `docs/RELEASE-mac.md` runbook; CI skeleton. **Verified by a real unsigned `--dir` build** (correct layout, bundled interpreter imports the bundled backend). Adversarially reviewed (6 lenses; fixed dmg-notarization, auto-update crash, redundant sign hook). **Still needs from Het:** an Apple Developer account ($99/yr) + Developer ID cert to actually sign/notarize, a `build/icon.icns`, and the real GitHub `owner/repo` in `build.publish`. **Remaining gap:** the agent work-root (editor works packaged; agent's bare-relative `projects/` writes need a writable work-root) + bundle-size trim (~950MB unpacked).
- **Lane E — bootstrapper/doctor/packs: DONE + committed** (`f6ba077`). First launch builds a managed venv via bundled `uv` (`lib/provision.py` engine, stdlib-only; `scripts/provision.py` NDJSON CLI), installs core deps wheels-only + ffmpeg, atomic + manifest staleness. `main.js` runs core provisioning in a native setup window BEFORE the backend, then `pythonBin()` prefers the venv. Lazy capability packs (transcription/vision/bg-removal/beat-sync/tts) via `POST /api/provision/{pack}`; `GET /api/doctor` status; `OPENNOLAN_FORCE_PROVISION` dogfood flag. `fetch-uv.mjs` bundles a sha-verified uv. **Verified end-to-end**: real venv build with the bundled interpreter → backend boots from it; `--dir` build confirms uv/provision.py/setup.html land in the bundle. **Remaining:** pin the ffmpeg source URL+sha; wire the lazy-pack **409 gate** into the agent tool path (so an agent tool needing an absent pack auto-prompts install); verify the first-run GUI flow on a clean Mac (or via `OPENNOLAN_FORCE_PROVISION`).
