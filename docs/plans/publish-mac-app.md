...
- **Lane E — bootstrapper/doctor/packs:** depends on D's bundle layout. Must also make `pythonBin()` prefer the managed venv under `OPENNOLAN_HOME/runtime` (the bundled base interpreter has no site-packages, so the packaged backend can't `import fastapi` until the venv is provisioned).
