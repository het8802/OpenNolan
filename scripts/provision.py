...
def main() -> int:
    ...
    # OPN-10: Electron spawns this script DIRECTLY (main.js) — it never imports
    # server.app, so first-run provisioning (uv venv/pip, npm ci: the biggest
    # cache writers) needs its own routing call or those caches land in ~/.npm
    # and ~/.cache. Same gate and idempotence sentinel as the backend's call.
