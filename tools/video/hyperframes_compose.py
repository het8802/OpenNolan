...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        """Materialize the HyperFrames workspace from OpenNolan artifacts.
        ...
        html = self._generate_index_html(
            ...
            or f"OpenNolan {edit_decisions.get('renderer_family', 'composition')}",
        )
    ...
    def _style_bridge(
        ...
        """Bridge OpenNolan playbook → HyperFrames CSS vars + DESIGN.md.
        ...
        design_md = (
            "# DESIGN\n\n"
            "Generated by OpenNolan HyperFrames style bridge (fallback).\n\n"
            f"- Background: `{bg}`\n"
            f"- Foreground: `{fg}`\n"
            f"- Accent: `{accent}`\n"
            f"- Primary: `{primary}`\n"
