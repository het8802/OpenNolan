...
5. Save ordered PNG slides (`slide-01.png`, `slide-02.png`, …). If native Codex produces a non-Instagram size, keep the original and resize/crop only as a transparent post-processing step if needed.
6. Generate a contact sheet.
7. QA with vision for mobile readability, cropping, spelling, story order, and whether it looks premium rather than HTML.
8. Fix issues before delivery.
9. Deliver individual slides in exact carousel order first. Include a contact sheet and caption/CTA. Do not make the user download a zip just to review the post; a zip/archive is optional only as a secondary backup when useful.
