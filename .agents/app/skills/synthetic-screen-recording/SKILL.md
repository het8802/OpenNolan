...
description: "Synthesize terminal/CLI demos in Remotion with TerminalScene instead of capturing the real desktop. Use when a demo shows a terminal, CLI tool, install flow, or coding workflow and the command/output sequence can be authored in advance; capture live only for real app UIs or unpredictable behavior."
---
...
install_steps = [
    ...
    {"kind": "cmd", "text": "git clone https://github.com/calesthio/OpenNolan.git",
     "typeSpeed": 0.045, "holdSeconds": 0.3},
    {"kind": "out", "text": "Cloning into 'OpenNolan'..."},
...
cuts.append({
    "id": "install-terminal",
    "type": "terminal_scene",
    "terminalTitle": "bash — OpenNolan setup",
    "prompt": "$",
    "accentColor": "#22D3EE",
    "steps": install_steps,
    "in_seconds": 50.0,
...
The `.agents/app/skills/synthetic-screen-recording` pattern also owns `ProviderChip` — a rotating badge overlay that cycles through a list of provider names at a fixed cadence. Used in the v3 showcase to cycle through all 11 AI video-gen providers during the "generated motion" section.
...
- `.agents/app/skills/remotion` — general Remotion authoring (hooks, springs, sequences)
- `.agents/app/skills/playwright-recording` — real browser-flow capture for web apps
...
Introduced: OpenNolan showcase v3 render (2026-04-16). Original motivation: the v3 setup walkthrough section needed a 60-second install demo where every command aligned to Chirp 3 HD narration cues, and Windows-MCP-driven real capture was too flaky in practice. See `projects/opennolan-showcase/build_composition.py` for the reference implementation.
